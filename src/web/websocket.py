...
class WebSocketManager:
    ...
    async def connect(self, websocket: WebSocket, session_id: str):
        ...
        logger.info(f"WebSocket 連接已建立: {session_id}")
    ...
    def disconnect(self, session_id: str):
        ...
        if session_id in self.active_connections:
            # 停止心跳任務
            self.stop_heartbeat(session_id)
            # 移除連接
            del self.active_connections[session_id]
            logger.info(f"WebSocket 連接已關閉: {session_id}")

    async def broadcast_progress(self, session_id: str, progress: dict[str, Any]):
        """發送進度更新到特定session"""
        if session_id not in self.active_connections:
            return

        try:
            await self.active_connections[session_id].send_json({"type": "progress", "data": progress})
            logger.debug(f"進度更新已發送: {session_id}")
        except Exception as e:
            logger.error(f"發送進度更新失敗: {e}")
            # 連接可能已斷開，移除連接
            self.disconnect(session_id)

    async def broadcast_chat_message(self, session_id: str, message: dict[str, Any]):
        """發送聊天消息到特定session"""
        if session_id not in self.active_connections:
            return

        try:
            await self.active_connections[session_id].send_json({"type": "chat_message", "data": message})
            logger.debug(f"聊天消息已發送[{session_id[:5]}]: {message.get('content')[:20]}")
        except Exception as e:
            logger.error(f"發送聊天消息失敗: {e}")
            # 連接可能已斷開，移除連接
            self.disconnect(session_id)

    def start_heartbeat(self, session_id: str):
        """啟動心跳任務"""
        ...
        # 創建新的心跳任務
        self.heartbeat_tasks[session_id] = asyncio.create_task(self._heartbeat_loop(session_id))
        logger.debug(f"心跳任務已啟動: {session_id}")

    def stop_heartbeat(self, session_id: str):
        """停止心跳任務"""
        if session_id in self.heartbeat_tasks:
            self.heartbeat_tasks[session_id].cancel()
            del self.heartbeat_tasks[session_id]
            logger.debug(f"心跳任務已停止: {session_id}")
    ...
    async def _heartbeat_loop(self, session_id: str):
        ...
        try:
            ...
            logger.debug(f"心跳循環已取消: {session_id}")
        except Exception as e:
    ...
    async def _send_heartbeat(self, session_id: str):
        ...
        try:
            ...
            logger.debug(f"心跳訊息已發送: {session_id}")
        except Exception as e:
            logger.error(f"發送心跳訊息失敗: {e}")
            # 連接可能已斷開，移除連接
            self.disconnect(session_id)

    async def send_stream_response(self, session_id: str, response: StreamChatResponse | dict[str, Any]):
        """發送流式回應"""
        if session_id not in self.active_connections:
            return
    ...
    async def broadcast_text(self, session_id: str, message: str):
        """發送文本消息"""
        if session_id not in self.active_connections:
            return
